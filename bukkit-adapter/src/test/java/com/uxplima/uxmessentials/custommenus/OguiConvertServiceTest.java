...
/**
 * Pure coverage of the file-facing OGUI convert service: it turns an OGUI {@code .yml} (or a directory of them) under
...
class OguiConvertServiceTest {
    ...
    void convertsASingleFileAndWritesTheConf(@TempDir Path menus) throws Exception {
        ...
        OguiConvertService service = new OguiConvertService(menus, new OguiConverter(), log);
        ...
        assertThat(Files.readString(menus.resolve("shop.conf"))).contains("title=", "rows=4");
    }

    @Test
    void convertsEveryYmlInADirectory(@TempDir Path menus) throws Exception {
        Path source = Files.createDirectory(menus.resolve("ogui"));
        Files.writeString(source.resolve("one.yml"), MENU);
        Files.writeString(source.resolve("two.yaml"), MENU);
        OguiConvertService service = new OguiConvertService(menus, new OguiConverter(), log);

        ConvertReport report = service.convert("ogui");
    ...
    void countsAnUnparsableFileAsSkippedWithoutCrashing(@TempDir Path menus) throws Exception {
        ...
        Files.writeString(menus.resolve("bad.yml"), "shop:\n  title: 'x'\n\tbroken: [oops\n");
        OguiConvertService service = new OguiConvertService(menus, new OguiConverter(), log);
